...
export const tool = {
  name: 'shell_exec_sudo',
  description: 'Execute a shell command with sudo on the VM via SSH — use for privileged operations',
  inputSchema: {
  ...
  handler: async (input: { command?: string; timeout?: number }) => {
    console.error('[shell_exec_sudo] Handler called with:', {
    ...
    try {
      const result = await execCommand(input.command, { timeout, sudo: true });
      return {
        success: true,
      ...
      console.error('[shell_exec_sudo] Error:', message);
